...
2. Linear Flight Path (constant velocity cross-track)
3. Orbital Arc (LEO satellite overpass)
...
6. Evasive / Erratic Trajectory (high-g maneuvers, evasion, random-walk)
7. Occlusion & Re-acquisition (target fade / signal dropout)
"""
...
def list_scenarios() -> List[Scenario]:
    """Return all available scenarios in display order."""
    return list(SCENARIO_REGISTRY.values())


def get_scenario_by_id_or_key(key: str) -> Optional[Scenario]:
    """Look up scenario by numeric key ('1'-'6') or scenario ID ('stationary', etc.)."""
